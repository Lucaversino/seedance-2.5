...
export default function App() {
  ...
  const readJson = async (res: Response) => {
    ...
    catch { throw new Error(`O servidor retornou uma resposta inválida (HTTP ${res.status}).`); }
  };

  const runGeneration = async (event?: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || data.message || `Falha ao criar tarefa (HTTP ${res.status}).`);
      ...
      for (let i=0; i<120; i++) {
        ...
        if (status === 'COMPLETED') {
          ...
          const result = await readJson(resultRes);
        ...
        <div className="grid gap-6 lg:grid-cols-[.9fr_1.1fr]">
          <section className="space-y-6">
            <div className="rounded-3xl border border-white/7 bg-[#0d0f1b] p-5 md:p-6">
              ...
              <button type="button" onClick={testConfig} className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl border border-slate-700 bg-slate-900 px-4 py-3 text-sm font-bold hover:border-slate-600"><Activity className="h-4 w-4"/>Verificar API</button>
            </div>
            ...
            <div className="rounded-3xl border border-white/7 bg-[#0d0f1b] p-5 md:p-6">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div><label className={label}>Áudio nativo</label><button type="button" onClick={()=>setConfig({...config,generateAudio:!config.generateAudio})} className={`flex w-full items-center justify-between rounded-xl border px-4 py-3 text-sm ${config.generateAudio?'border-violet-500/30 bg-violet-500/10 text-violet-200':'border-slate-700 bg-slate-950 text-slate-400'}`}><span className="flex items-center gap-2"><Volume2 className="h-4 w-4"/>{config.generateAudio?'Ativado':'Desativado'}</span><span className={`h-5 w-9 rounded-full p-0.5 ${config.generateAudio?'bg-violet-500':'bg-slate-700'}`}><span className={`block h-4 w-4 rounded-full bg-white transition ${config.generateAudio?'translate-x-4':'translate-x-0'}`}/></span></button></div>
              </div>
              <button type="button" onClick={saveConfig} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-violet-600 px-4 py-3 text-sm font-bold hover:bg-violet-500"><Save className="h-4 w-4"/>Salvar padrões</button>
            </div>
          </section>
          ...
          <section className="space-y-6">
            <div className="rounded-3xl border border-white/7 bg-[#0d0f1b] p-5 md:p-6">
              ...
              <div className="mb-5 grid grid-cols-2 rounded-xl bg-slate-950 p-1"><button type="button" onClick={()=>setMode('text')} className={`flex items-center justify-center gap-2 rounded-lg py-2.5 text-xs font-bold ${mode==='text'?'bg-violet-600 text-white':'text-slate-500'}`}><Terminal className="h-4 w-4"/>Texto → Vídeo</button><button type="button" onClick={()=>setMode('image')} className={`flex items-center justify-center gap-2 rounded-lg py-2.5 text-xs font-bold ${mode==='image'?'bg-violet-600 text-white':'text-slate-500'}`}><ImageIcon className="h-4 w-4"/>Imagem → Vídeo</button></div>
